 ...
 *
 * - If Origin is missing (same-origin or server-to-server) → allowed
 * - If Origin matches the request URL's origin (same host) → allowed
 * - If Origin is in the allowlist → allowed
 * - Otherwise → ok = false (disallow cross-origin from unknown hosts)
 */
export function validateOrigin(request) {
  const origin = request.headers.get("origin");
  const defaultOrigin = ALLOWED_ORIGINS[0];

  // Same-origin: derive request origin from URL (so /propfirms on any domain works)
  ...
  const allowedOrigin = origin || requestOrigin || defaultOrigin;
...
/** Clear in-memory rate limit store (for tests). */
