...
export function processOutgoingTransactions(nativeData, tokenData, sourceAddresses, days = 7) {
  ...
  const allTxs = [...nativeTxs, ...tokenTxs]
    .map(tx => ({
      ...
      fromShort: `${tx.from.slice(0, 6)}...${tx.from.slice(-4)}`,
      arbiscanUrl: `https://arbiscan.io/tx/${tx.txHash}`,
      paymentMethod: tx.token === 'RISEPAY' ? 'Rise' : 'Crypto', // Map RISEPAY to Rise
    }))
    .filter(tx => tx.amountUSD >= 10); // Filter out transactions < $10 (spam)

  // Deduplicate by transaction hash (in case multiple addresses are involved in same tx)
...
export function groupByDay(transactions, days = 7) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const riseAmount = dayTxs
      .filter(tx => tx.paymentMethod === 'Rise')
      .reduce((sum, tx) => sum + tx.amountUSD, 0);
    const cryptoAmount = dayTxs
      .filter(tx => tx.paymentMethod === 'Crypto')
      .reduce((sum, tx) => sum + tx.amountUSD, 0);
    const wireAmount = dayTxs
      .filter(tx => tx.paymentMethod === 'Wire Transfer')
      .reduce((sum, tx) => sum + tx.amountUSD, 0);

    dailyData.push({
      date: d.toISOString().split('T')[0],
      totalUSD: Math.round(totalUSD),
      rise: Math.round(riseAmount),
      crypto: Math.round(cryptoAmount),
      wireTransfer: Math.round(wireAmount),
    });
  }
