 ...
 * sendAlert(service, message, severity [, details]) sends to ADMIN_ALERT_EMAILS (or fallback).
 * No-op if no recipient email or RESEND_API_KEY is not set.
 */
...
/**
 * Return the list of admin alert recipient addresses.
 *
 * Fallback chain:
 *   1. ADMIN_ALERT_EMAILS (comma-separated) if set and non-empty
 *   2. ALERT_EMAIL (single address) if set   // @deprecated — use ADMIN_ALERT_EMAILS instead
 *   3. Hardcoded default ['jasonusca@gmail.com']
 *
 * @returns {string[]}
 */
export function getAdminAlertEmails() {
  ...
  // @deprecated — use ADMIN_ALERT_EMAILS instead
  const legacy = process.env.ALERT_EMAIL;
  if (legacy && legacy.trim()) {
    return [legacy.trim()];
  }

  return ['jasonusca@gmail.com'];
}
...
export async function sendAlert(service, message, severity, details = {}) {
  ...
  if (recipients.length === 0) {
    console.warn('[alerts] No recipient emails configured; alert not sent:', { service, message, level });
    return;
  }
