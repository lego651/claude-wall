...
export class ArbiscanCircuitBreaker {
  ...
  async execute(fn) {
    ...
    if (this.state === CIRCUIT_STATE.OPEN) {
      ...
      if (typeof console.warn === 'function') {
        console.warn('[Arbiscan] Circuit breaker open, request blocked');
      }
      throw err;
    }
  ...
  _maybeTransitionFromOpen() {
    ...
    if (now >= this.nextAttemptAllowedAt) {
      ...
      this.failureCount = 0;
      if (typeof console.warn === 'function') {
        console.warn('[Arbiscan] Circuit breaker half-open, allowing trial request');
      }
    }
  }
  ...
  _recordSuccess() {
    if (this.state === CIRCUIT_STATE.HALF_OPEN) {
      ...
      this.failureCount = 0;
      if (typeof console.warn === 'function') {
        console.warn('[Arbiscan] Circuit breaker closed after successful trial');
      }
    } else {
      this.failureCount = 0;
    }
  ...
  _trip() {
    ...
    if (typeof console.warn === 'function') {
      console.warn(
        `[Arbiscan] Circuit breaker opened after ${this.failureCount} failures, retry after ${this.resetTimeoutMs}ms`
      );
    }
  }
