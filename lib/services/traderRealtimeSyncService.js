...
function processTraderPayouts(nativeData, tokenData, walletAddress) {
  ...
  const cutoff7d = now - 20 * 24 * 60 * 60;
  const walletLower = walletAddress.toLowerCase();

  const nativePayouts = (nativeData || [])
    .filter(
      (tx) =>
        tx.to && tx.to.toLowerCase() === walletLower && parseInt(tx.timeStamp) >= cutoff7d
    )
    .map((tx) => {
  ...
  const tokenPayouts = (tokenData || [])
    .filter(
      (tx) =>
        ...
        parseInt(tx.timeStamp) >= cutoff7d &&
 ...
 */
export async function cleanupOldTraderPayouts(hoursToKeep = 480) {
  const supabase = createServiceClient();
