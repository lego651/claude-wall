...
describe('payoutDataLoader', () => {
  ...
  describe('loadMonthlyData', () => {
    it('loads existing file successfully', () => {
      const firmId = 'firm1';
      ...
      const result = loadMonthlyData(firmId, yearMonth);
      expect(result).toEqual(data);
    });

    it('returns null for missing file', () => {
      const result = loadMonthlyData('nofirm', '2025-01');
      expect(result).toBeNull();
    });

    it('returns null for missing directory (existsSync false)', () => {
      mockDirContents.clear();
      mockFileContent.clear();
      const result = loadMonthlyData('nodir', '2025-01');
      expect(result).toBeNull();
    });
    ...
    it('handles corrupted JSON gracefully', () => {
      ...
      const result = loadMonthlyData('firm1', '2025-01');
      expect(result).toBeNull();
    });
    ...
    it('logs warning when file size exceeds 500KB', () => {
      const filePath = path.join(PAYOUTS_BASE, 'firm1', '2025-01.json');
  ...
  describe('loadPeriodData', () => {
    ...
    it('filters to last 7 days correctly', () => {
      ...
      const result = loadPeriodData('firm1', '7d');
    ...
    it('filters to last 30 days correctly', () => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date('2025-02-15T12:00:00.000Z'));
      const currentMonth = '2025-02';
      const prevMonth = '2025-01';
      ...
      const result = loadPeriodData('firm1', '30d');
      expect(result.summary.payoutCount).toBe(2);
    ...
    it('aggregates 12 months correctly', () => {
      ...
      const result = loadPeriodData('firm1', '12m');
    ...
    it('merges data from multiple months for 30d', () => {
      jest.useFakeTimers();
    ...
    it('returns empty summary for unknown period', () => {
      const result = loadPeriodData('firm1', '90d');
      expect(result).toEqual({ summary: {}, dailyBuckets: [], monthlyBuckets: [] });
    });
  });

  describe('getTopPayoutsFromFiles', () => {
    it('returns top N payouts sorted by amount', () => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date('2025-02-15T12:00:00.000Z'));
      const currentPath = path.join(PAYOUTS_BASE, 'firm1', '2025-02.json');
      const prevPath = path.join(PAYOUTS_BASE, 'firm1', '2025-01.json');
      ...
      const result = getTopPayoutsFromFiles('firm1', '30d', 2);
    ...
    it('filters by period 30d', () => {
      ...
      const result = getTopPayoutsFromFiles('firm1', '30d', 10);
      expect(result.length).toBe(1);
      expect(result[0].amount).toBe(100);
      jest.useRealTimers();
    });

    it('filters by period 12m', () => {
      const firmDir = path.join(PAYOUTS_BASE, 'firm1');
      ...
      const result = getTopPayoutsFromFiles('firm1', '12m', 5);
      expect(result.length).toBe(2);
    ...
    it('handles empty data', () => {
      ...
      expect(getTopPayoutsFromFiles('firm1', '30d', 10)).toEqual([]);
      expect(getTopPayoutsFromFiles('firm1', '99d')).toEqual([]);
      jest.useRealTimers();
    });
  });
});
