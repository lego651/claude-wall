...
import { fetchNativeTransactions, fetchTokenTransactions, usageTracker } from '@/lib/arbiscan';
...
export async function syncFirmPayouts(firm) {
  ...
  try {
    ...
    const { error: upsertError } = await withQueryGuard(
      supabase.from('firm_recent_payouts').upsert(payouts, { onConflict: 'tx_hash' }),
      { context: 'sync recent_payouts upsert' }
    );
...
export async function updateFirmLastPayout(firmId, latestPayout) {
  ...
  const { data: firm } = await withQueryGuard(
    supabase.from('firm_profiles').select('last_payout_at').eq('id', firmId).single(),
    { context: 'updateFirmLastPayout firms select' }
  );
  ...
  if (!existingTimestamp || newTimestamp > existingTimestamp) {
    const { error } = await withQueryGuard(
      supabase
        .from('firm_profiles')
        .update({
        ...
        .eq('id', firmId),
      { context: 'updateFirmLastPayout firms update' }
    );
    ...
    await withQueryGuard(
      supabase
        ...
        .eq('id', firmId),
      { context: 'updateFirmLastPayout firms update sync' }
    );
  }
}
...
export async function cleanupOldPayouts(hoursToKeep = 24) {
  ...
  const { error, count } = await withQueryGuard(
    supabase
      .from('firm_recent_payouts')
      .delete()
      .lt('timestamp', cutoffDate)
      .select('*', { count: 'exact', head: true }),
    { context: 'cleanupOldPayouts recent_payouts' }
  );
...
export async function syncAllFirms() {
  ...
  const { data: firms, error: fetchError } = await withQueryGuard(
    supabase.from('firm_profiles').select('id, name, addresses'),
    { context: 'syncAllFirms firms' }
  );
  ...
  const arbiscanUsage = typeof usageTracker?.getUsage === 'function' ? usageTracker.getUsage() : null;
  if (arbiscanUsage) {
    log.info(
      { arbiscanCalls: arbiscanUsage.calls, arbiscanLimit: arbiscanUsage.limit, arbiscanPercentage: arbiscanUsage.percentage, day: arbiscanUsage.day },
      'Arbiscan usage'
    );
  }
