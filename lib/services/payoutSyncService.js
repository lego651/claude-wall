...
export async function syncFirmPayouts(firm) {
  ...
  if (!apiKey) {
    // Log context to debug Inngest/Vercel env (which deployment is running)
    const vercelEnv = process.env.VERCEL_ENV || 'unknown';
    const vercelUrl = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'unknown';
    console.error(
      '[Sync] Missing ARBISCAN_API_KEY. Vercel env:',
      vercelEnv,
      'URL:',
      vercelUrl,
      '- Ensure this deployment has ARBISCAN_API_KEY in Vercel (see docs/INNGEST-ENV-VARS.md)'
    );
...
export async function syncAllFirms() {
  ...
  if (!firms || firms.length === 0) {
    console.log('[Sync] No firms found in database');
    return { firms: 0, totalPayouts: 0, errors: [], duration: 0 };
  }

  console.log(`[Sync] Found ${firms.length} firms to sync`);
  ...
  console.log(`[Sync] Complete! ${summary.totalPayouts} payouts synced in ${summary.duration}ms`);
  if (summary.errors.length > 0) {
    console.log(`[Sync] Errors:`, summary.errors);
  }
