...
// In-memory counters for dashboard (PROP-021). Reset on process restart.
...
/**
 * Return cache hit/miss counts (in-memory, since process start).
 * @returns {{ hits: number, misses: number, hitRate: number | null }}
 */
export function getCacheStats() {
  const { hits, misses } = _stats;
...
export const cache = { get, set, invalidate, getCacheStats };
