...
**Y-axis price labels (right side of chart):**
- GREEN label on Y-axis = TAKE PROFIT level — always extract this as take_profit
- RED label on Y-axis = STOP LOSS level — always extract this as stop_loss
- BLACK/DARK label on Y-axis = current market price or key level — do NOT use as entry unless confirmed by a trade box

**Trade position boxes (Long/Short Position tool):**
- A colored rectangle spanning price levels = a trade setup
- The ENTRY line = the horizontal boundary between the green zone (profit) and red zone (loss) inside the box
- The top of a LONG box (green zone ceiling) = take profit
- The bottom of a LONG box (red zone floor) = stop loss
- Do NOT confuse the current price ticker (live price label) with the entry price

**Horizontal lines drawn on chart:**
- Red horizontal line = stop loss
- Green horizontal line = take profit
- Gray/white/black horizontal line = support/resistance or entry level

**Entry price**: read from the trade box dividing line, an explicit entry annotation, or a gray horizontal line — never from the live price ticker on the Y-axis.

**Read price values precisely**: zoom into Y-axis labels carefully — e.g. 4,551.83 ≠ 4,553.13. Extract the exact digits shown.
