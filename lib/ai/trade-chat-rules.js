...
**Y-axis price labels (right side of chart) — the only source for entry/SL/TP:**
...
- GRAY-background label on Y-axis = ENTRY PRICE — extract exact value including all decimals
- BLACK/DARK label on Y-axis = current live market price — NEVER use as entry, SL, or TP
...
**When multiple charts are stacked (e.g. 1H on top, 5m on bottom):** focus on the chart that shows the trade setup box or drawn levels — usually the lower/more zoomed-in chart. Read its Y-axis labels, not the upper chart's.

**Logical validation — always cross-check before returning:**
- BUY/LONG trade: stop_loss MUST be less than entry_price; take_profit MUST be greater than entry_price
- SELL/SHORT trade: stop_loss MUST be greater than entry_price; take_profit MUST be less than entry_price
- If your extracted values violate this, re-examine the chart and correct them

**Read all prices with full decimal precision.** The Y-axis labels show exact values — read every digit. Do not round or approximate.
