...
- **Minimal complexity** - No caching, simple error handling, basic USD conversion
...
### APIs & Services
- **Arbiscan API** - Transaction data only
- **Simple USD Conversion** - Use constant ETH price (~$2500) and stablecoins = $1.00
  - ⚠️ **TODO (Future):** Implement historical price lookup via CoinGecko for accuracy
...
- Convert amounts to USD using **simple constant prices**:
  - ETH = $2500 (constant)
  - USDC/USDT = $1.00 (stablecoin)
  - ⚠️ **TODO:** Upgrade to historical prices in Phase 1
- Filter out transactions < $10 (spam/dust)
- Return last 100 transactions or 6 months of data (whichever is smaller)
- **No caching** - Direct API calls each time (simple for v0)
...
3. `convertToUSD(amount, token)` - Simple constant price conversion (no API needed)
   - ETH → amount * 2500
   - USDC/USDT → amount * 1.00
4. `filterAndSort(transactions)` - Remove dust, sort by date
5. `calculateStats(transactions)` - Compute totals and 30-day metrics

**Caching Strategy:**
- ❌ **No caching for v0** - Keep it simple, fetch fresh data each request
- Can add caching in Phase 1 if rate limits become an issue

### Error Handling
- **Minimal error handling** for v0:
  - If Arbiscan API fails → return error message to frontend
  - Log errors to console
  - No retries, no fallbacks, no complex error recovery
- Errors will help us understand issues during testing
...
## Timeline Estimate
- **API Development:** 1-2 days (simplified, no caching/complex errors)
- **UI Integration:** 1 day
- **Testing & Refinement:** 0.5 day
- **Total:** ~2-3 days for v0
...
- **Simple USD conversion** is acceptable for v0 testing (not production-ready)
...
1. **Historical USD prices** - Replace constant prices with CoinGecko historical API
2. **Caching** - Add 5-minute cache to reduce Arbiscan API calls
3. **Error handling** - Implement retry logic, fallbacks, proper error recovery
4. **Rate limiting** - Handle Arbiscan API rate limits gracefully
5. **Performance** - Optimize for faster load times if needed
