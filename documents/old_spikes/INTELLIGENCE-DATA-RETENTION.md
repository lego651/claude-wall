...
**Note:** Retention cleanup now runs inside the scrape job (`scripts/backfill-firm-trustpilot-reviews.ts`), not as a separate workflow. See `documents/runbooks/trustpilot-scraper-faq.md`.
...
- **Manual backfill** (e.g. `npx tsx scripts/backfill-firm-trustpilot-reviews.ts`) uses default 6 pages and no review cap unless you set `TRUSTPILOT_BACKFILL_PAGES` and/or `TRUSTPILOT_MAX_REVIEWS` in the environment.
