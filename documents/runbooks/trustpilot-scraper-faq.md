...
- **Where:** The daily scrape workflow (`.github/workflows/sync-trustpilot-reviews.yml`) runs `scripts/backfill-trustpilot.ts`. That script (1) scrapes all firms, (2) then runs **retention cleanup** in the same process:
