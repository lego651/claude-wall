...
function SortableAccountRow({ account, children }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex flex-col gap-2 px-4 py-3 border rounded-xl bg-white ${account.is_default ? "border-slate-300" : "border-slate-200"}`}
    >
...
export default function TradingLogSettings() {
  ...
  async function handleReorder({ active, over }) {
    ...
    try {
      const res = await fetch("/api/trade-accounts/reorder", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
